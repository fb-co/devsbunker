<template>
    ...
    <div class="query_input_container" @keyup.stop="nextDocument">
        <!-- Stop the event propogation for the keyup function -->
        ...
        <div class="main_query_results" ref="results">
            <div v-if="searchFor === 'users'">
                <p
                    @mousedown="addEntry(document.username)"
                    v-for="(document, index) in documents"
                    :key="document.username"
                    class="document_item"
            ...
            <div v-else>
                <p
                    @mousedown="addEntry(document.name)"
                    v-for="(document, index) in documents"
                    :key="document.name"
                    class="document_item"
        ...
        queryData() {
            if (this.$refs.input_ref.value != "") {
                if (this.searchFor === "users") {
                    // include all searches that require a request to the server in 'or' format at this if statement
                    if (!this.queryQueued) {
