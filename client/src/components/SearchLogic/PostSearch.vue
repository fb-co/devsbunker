...
export default {
    ...
    methods: {
            ...
            if (this.$refs.general_input.value != "" && this.$refs.general_input.value.length > 2) {
                if (!this.queryQueued && !this.typing) {
                    ...
                    if (postsInCache) {
                        ...
                        }, 100); // need small wait for cards to render
                        ...
                        setTimeout(() => {
                            if (this.$refs.general_input.value != "") {
                                GraphQLPostsService.fetchPostsByPartial(
        ...
        async loadMoreResults() {
            const res = await GraphQLPostsService.fetchPostsByPartial(
                this.$refs.general_input.value,
                this.filter,
                this.userToFilter,
                this.sortingType,
        ...
        addToInput(value) {
            ...
            }, 100); // since the binding is on keydown you need to do this (I know its a little janky)
        },
    },
};
</script>
